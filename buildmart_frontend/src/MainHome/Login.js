    ...
    fetch("http://localhost:8080/checkLogin", reqOptions)
      ...
      .then((obj) => {
        if (Object.keys(obj).length === 0) {
          ...
          if (obj.role.id === 1 && obj.active_status === "Active") {
            localStorage.setItem("CustomerUser",JSON.stringify(obj.customer));
            navigate("/customer");
          } else if (obj.role.id === 2 && obj.active_status === "Active") {
            localStorage.setItem("CompanyUser",JSON.stringify(obj.company));
            navigate("/company");
          } else if (obj.role.id === 3 && obj.active_status === "Active") {
            localStorage.setItem("VendorUser",JSON.stringify(obj.vendor));
            navigate("/vendor");
          } else if (obj.role.id === 4 && obj.active_status === "Active") {
